...
function createCard(domain_name, httpCon) {
  fetch(
    `https://api.ip2whois.com/v1?key=${your_license_key}&domain=${domain_name}`,
    {
      method: "GET",
      mode: "no-cors",
      headers: {
        ...
        "Access-Control-Allow-Origin":
          "http://localhost:5500/Tasks/Week5/api-2/api2/",
      },
    }
  )
    .then(function (resp) {
    ...
    .then(function (data) {
      ...
      if (errorCode != "") {
        ...
        let expireDate =
          data.expire_date === "" ? ["∞"] : data.expire_date.split("-");
  ...
  if (days != 0) {
    totalDay =
      days === 1
        ...
        : `<span class="font-weight-bold">${days}</span> days `;
  }
  if (months != 0) {
    totalMonth =
      months === 1
        ? `<span class="font-weight-bold">${months}</span> month `
        : `<span class="font-weight-bold">${months}</span> months `;
  }
  if (years != 0) {
    totalYear =
      years === 1
        ? `<span class="font-weight-bold">${years}</span> year `
        : `<span class="font-weight-bold">${years}</span> years `;
  }
